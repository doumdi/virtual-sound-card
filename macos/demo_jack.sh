#!/bin/bash
# Demo script for macOS Virtual Sound Card with JACK2
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}macOS Virtual Sound Card - JACK2 Demo${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
if ! jack_lsp >/dev/null 2>&1; then
    ...
    echo ""
    echo "1. Start JACK with CoreAudio:"
    echo "   ${BLUE}jackd -d coreaudio &${NC}"
    echo ""
    echo "2. Use QjackCtl GUI (recommended):"
    echo "   ${BLUE}qjackctl${NC}"
    echo ""
    echo "   Install QjackCtl with: ${BLUE}brew install qjackctl${NC}"
    echo ""
    exit 1
fi
